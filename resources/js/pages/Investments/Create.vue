<template>
  <AppLayout title="Add Investment">
    <div class="space-y-6 p-4">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator><Slash /></BreadcrumbSeparator>
          <BreadcrumbItem>
            <BreadcrumbLink :href="route('businesses.investments.index', business.id)">Investments</BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator><Slash /></BreadcrumbSeparator>
          <BreadcrumbItem>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Add a new investment to {{ business.name }}</CardDescription>
        </CardHeader>
        <CardContent>
          <form @submit.prevent="submit" class="space-y-4">
            ...
            <div class="grid gap-2">
              <Label for="invested_at">Investment Date</Label>
              <Popover>
                <PopoverTrigger as-child>
                  <Button
                    ...
                    :class="cn(
                      ...
                      !form.invested_at && 'text-muted-foreground',
                    )"
                  >
                    <span>{{ form.invested_at || 'Pick a date' }}</span>
                ...
                <PopoverContent class="w-auto p-0">
                  <Calendar
                    ...
                    :class="{ 'border-destructive': form.errors.invested_at }"
                  />
                </PopoverContent>
              </Popover>
              <InputError :message="form.errors.invested_at" class="mt-2" />
            </div>
          </form>
        </CardContent>
        <CardFooter class="flex items-center gap-4 justify-end">
          ...
          >
            ...
            {{ form.processing ? 'Adding...' : 'Add Investment' }}
          </Button>
        </CardFooter>
      </Card>
    </div>
...
import { Slash, DollarSign, Loader2, Sparkles } from 'lucide-vue-next'
...
const submit = () => {
  form.post(route('businesses.investments.store', props.business.id))
}
...
const handleCancel = () => {
  if (window.history.length > 1) {
    ...
    window.location.href = route('businesses.investments.index', props.business.id);
  }
}
</script> 